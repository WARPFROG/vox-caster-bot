...
- The two feeds have deliberately different windows because the endpoints differ: `feedrecentchanges` takes `days` (set to 30), while `Special:NewPages` has no `days` param at all — it silently ignores one and reaches back to the `recentchanges` retention limit (`$wgRCMaxAge`, 90 days by default). Both read the same table. Keep the `days` value in the same ballpark as that retention, otherwise a downtime longer than the window drops edits for good while still recovering page creations (issue #24)
